...
final class WorkflowPlanCommandTest extends TestCase
{
    ...
    public function testEnterBuildsRecallInputFromOwnedArtifacts(): void
    {
        ...
        file_put_contents($root . '/.agent-loop/init.json', json_encode([
            'version' => 1,
            'recall' => ['document_manifests' => ['resources/recall-documents.json']],
        ], JSON_THROW_ON_ERROR));
        file_put_contents($root . '/resources/recall-documents.json', json_encode([
            'schema_version' => '1.0',
            'documents' => [],
        ], JSON_THROW_ON_ERROR));
...
CARD
        ...
        try {
            ...
            self::assertSame([[
                ...
                '--document-manifest', $root . '/resources/recall-documents.json',
    ...
    public function testEnterRejectsMissingConfiguredRecallDocumentManifestBeforeCompilation(): void
    {
        ...
        file_put_contents($root . '/.agent-loop/init.json', json_encode([
            'version' => 1,
            'recall' => ['document_manifests' => ['resources/missing.json']],
        ], JSON_THROW_ON_ERROR));
