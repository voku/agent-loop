...
final class AgentLoopVerifierTest extends TestCase
{
    ...
    public function testAnEphemeralSessionIsSkippedWhileGovernedWorkStillFailsTheSameGate(): void
    {
        ...
        file_put_contents($this->root . '/tasks/TASK-2.md', "# TASK-2\n");
    ...
    public function testRecallRootAutoDetectionAndCurrentFallback(): void
    {
        ...
        file_put_contents(
            $this->root . '/infra/doc/agent-learning/recall-output/current/meta.json',
            json_encode([
                'task_id' => 'TASK-1',
    ...
    public function testVerifyFailsWhenRunNoLongerMatchesCurrentApprovedContract(): void
    {
        ...
        $session = $sessions->create($this->root . '/session_plan', 'TASK-1', by: 'lars');
        ...
        file_put_contents($this->root . '/recall/TASK-1/meta.json', json_encode(['output_hashes' => []], JSON_THROW_ON_ERROR));
    ...
    private function writeSession(string $id, string $taskId, bool $ephemeral): void
    {
        ...
        foreach (['plan.md', 'assumptions.md', 'decisions.md', 'validation.md'] as $file) {
            file_put_contents($this->root . '/session_plan/' . $id . '/' . $file, '');
        }
        mkdir($this->root . '/session_plan/' . $id . '/checkpoints', 0o775, true);
    }
