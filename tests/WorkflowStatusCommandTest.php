...
final class WorkflowStatusCommandTest extends TestCase
{
    ...
    {
        (new SessionStore())->create($this->root.'/session_plan','ABC-123', by:'lars'); mkdir($this->root.'/recall/ABC-123',0775,true); file_put_contents($this->root.'/recall/ABC-123/meta.json','{}'); mkdir($this->root.'/.agent-recall/reviews',0775,true);
        foreach (['ok'=>'[OK] review', 'warn'=>'[WARN] review', 'fail'=>'[WARN] review'] as $status=>$needle) { file_put_contents($this->root.'/.agent-recall/reviews/ABC-123.blindspots.json', json_encode(['status'=>$status])); ob_start(); $exit=(new WorkflowStatusCommand($this->root))->run(['ABC-123']); $out=(string)ob_get_clean(); self::assertSame(0,$exit); self::assertStringContainsString('[OK] session: 1 session(s) found', $out); self::assertStringContainsString('[OK] recall: found', $out); self::assertStringContainsString($needle, $out); self::assertStringContainsString('status '.$status, $out); }
    }
