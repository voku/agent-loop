...
The package make fragment (`resources/make/agent-loop.mk`) reuses the same runtime. When the including Makefile defines no `AGENT_LOOP_RUN`, its `agent_workflow_*` and other targets run through the generated `$(AGENT_LOOP_GITHOOKS_DIR)/lib/agent-loop-hooks.sh`: in the declared container when it is reachable, otherwise on the host. Without a generated hook library they run directly on the host as before. A host-defined `AGENT_LOOP_RUN` always takes precedence. The command reaches the runtime as one quoted argument; nothing parses `next_action` or maps lifecycle actions to targets.
