...
- Codex policy uses project `.codex/rules/*.rules`; agent-loop-owned remote-publication shell commands are `forbidden` rather than `prompt`, so Auto-review is not an approval path for them. Project trust stays host/user-owned, and MCP/non-shell authority coverage is separate;
