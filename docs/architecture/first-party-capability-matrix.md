...
- Cursor shell authority policy projects one agent-loop-owned `beforeShellExecution` entry into `.cursor/hooks.json` and installs its repository-local guard with `failClosed: true`. Existing unrelated hook entries are preserved. Cursor CLI policy and MCP authority coverage are separate surfaces and remain unclaimed pending #616.
