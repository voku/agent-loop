...
- When a task has a durable Contract or task id, inspect `{{agent_loop_cli}} workflow status <task-id> --format=toon` before mutation and continue from persisted state rather than conversational memory.
