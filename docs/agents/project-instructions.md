...
1. Before mutating code, run `{{agent_loop_cli}} enter <task-id>`. If it reports `Mutation: ready`, use its bounded context to perform the approved host-native work; its `Next:` value is the next governed close-out action after that work and is not another mutation prerequisite. If `enter` exits non-zero, follow `Next:` to satisfy the returned prerequisite instead of bypassing it.
