...
5. **Same-intent premise challenge / replan** — an agent may choose a simpler implementation route while approved Goal, acceptance, scope, and authority remain unchanged; changing those boundaries returns to human authority. The package-owned simplicity/discipline skills define the implementation heuristic, while the lifecycle owns authority.
