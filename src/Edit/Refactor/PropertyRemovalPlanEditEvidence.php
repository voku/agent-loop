...
/** One exact deletion edit decoded from agent-map's property_removal_plan contract. */
final readonly class PropertyRemovalPlanEditEvidence
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        if ($edit->startFilePos < 0 || $edit->endFilePos < $edit->startFilePos) {
            throw new RuntimeException('Property removal edit contains an invalid byte range.');
        }
        if ($edit->lineStart < 1 || $edit->lineEnd < $edit->lineStart) {
            throw new RuntimeException('Property removal edit contains an invalid line range.');
        }
        if (preg_match('/\Asha256:[a-f0-9]{64}\z/D', $edit->sourceSha256) !== 1) {
            throw new RuntimeException('Property removal edit contains an invalid source SHA-256.');
        }
    ...
    private static function string(array $data, string $key): string
    {
        ...
        if (!is_string($value) || $value === '') {
            throw new RuntimeException('Property removal edit requires non-empty string ' . $key . '.');
        }
    ...
    private static function integer(array $data, string $key): int
    {
        $value = $data[$key] ?? null;
        if (!is_int($value)) {
            throw new RuntimeException('Property removal edit requires integer ' . $key . '.');
        }
