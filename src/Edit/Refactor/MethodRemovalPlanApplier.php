...
/** Applies one governed method-removal plan through a fail-closed transactional PHP edit boundary. */
final readonly class MethodRemovalPlanApplier
{
    ...
    public function apply(array $plan, AgentMapIndex $map, string $root): EditRunResult
    {
        ...
        try {
            ...
            if ($current['files'] !== $prepared['files'] || $current['source_hashes'] !== $prepared['source_hashes']) {
                throw new RuntimeException('Method removal source evidence changed while staged; no source was changed.');
            }
            ...
            if ($cleanupFailures !== []) {
                throw new RuntimeException(
                    'Method removal failed and staged-file cleanup was incomplete: ' . implode('; ', $cleanupFailures),
                    0,
                    $exception,
                );
            }
        ...
        return new EditRunResult(
            ...
            exitCode: 0,
            stdout: sprintf(
                "method_removal_plan applied %d deletion edit(s) across %d source file(s).\n",
                $prepared['edit_count'],
                count($prepared['files']),
            ),
            stderr: $warnings === [] ? '' : "Method removal completed with cleanup warning(s):\n- " . implode("\n- ", $warnings),
        );
    }
    ...
    {
        return $this->preflightDocument(MethodRemovalPlanDocument::fromArray($plan), $map, $root);
    }

    /**
     * @return array{files: array<string, string>, source_hashes: array<string, string>, plan_type: string, edit_count: int, move_count: int}
     */
    private function preflightDocument(MethodRemovalPlanDocument $plan, AgentMapIndex $map, string $root): array
    {
        $map = $this->withRuntimeRoot($map, $root);
        if ($map->staleEntries() !== []) {
            throw new RuntimeException('Current agent-map source evidence is stale; rebuild the map before applying a method removal plan.');
        }
        ...
        foreach ($plan->edits as $edit) {
            ...
            if (
                ...
                throw new RuntimeException('Method removal edit evidence changed before apply; rebuild and re-plan.');
            }
        ...
        foreach ($byFile as $path => $fileEdits) {
            ...
            foreach ($fileEdits as [$start, $end]) {
                if ($end >= $previousStart) {
                    throw new RuntimeException('Method removal plan contains overlapping edit ranges.');
                }
        ...
        try {
            foreach ($files as $path => $content) {
                ...
                if (!is_int($written) || $written !== strlen($content)) {
                    throw new RuntimeException('Unable to stage complete method-removal source: ' . $path);
                }
                $permissions = fileperms($path);
                if (!is_int($permissions) || !chmod($temporary, $permissions & 0o777)) {
                    throw new RuntimeException('Unable to preserve method-removal source permissions: ' . $path);
                }
            }
            ...
            foreach ($staged as $path => $temporary) {
                ...
                if ($lint['exit_code'] !== 0) {
                    ...
                    throw new RuntimeException(
                        'Rewritten PHP failed syntax validation before method-removal publication: '
                        . $path
                        . ($detail === '' ? '' : ' (' . $detail . ')'),
                    );
                }
            ...
            if ($cleanupFailures !== []) {
                throw new RuntimeException(
                    'Method-removal staging failed and cleanup was incomplete: ' . implode('; ', $cleanupFailures),
                    0,
                    $exception,
                );
            }
        ...
        try {
            foreach ($staged as $source => $temporary) {
                ...
                if (!$this->move($source, $backup)) {
                    throw new RuntimeException('Unable to back up method-removal source before publication: ' . $source);
                }
                ...
                if (!is_string($backupHash) || !hash_equals($sourceHashes[$source], 'sha256:' . $backupHash)) {
                    throw new RuntimeException('Method-removal source changed during publication precondition capture: ' . $source);
                }
                if (!$this->move($temporary, $source)) {
                    throw new RuntimeException('Unable to publish staged method-removal source: ' . $source);
                }
            ...
            if ($failures !== []) {
                throw new RuntimeException(
                    'Method-removal publication failed and rollback was incomplete: ' . implode('; ', $failures),
                    0,
                    $exception,
                );
            }
            throw new RuntimeException('Method-removal publication failed; every source file was restored.', 0, $exception);
        }
    ...
    private function rollback(array $backups, array $published): array
    {
        ...
        foreach (array_reverse($backups, true) as $source => $backup) {
            if (isset($published[$source]) && (is_file($source) || is_link($source)) && !unlink($source)) {
                $failures[] = 'unable to remove partially published method-removal source ' . $source;
            }
            if ((is_file($backup) || is_link($backup)) && !$this->move($backup, $source)) {
                $failures[] = 'unable to restore method-removal source backup ' . $source;
            }
        }
    ...
    private function cleanupPaths(array $paths): array
    {
        ...
        foreach ($paths as $path) {
            if ((is_file($path) || is_link($path)) && !unlink($path)) {
                $failures[] = 'unable to remove temporary method-removal file ' . $path;
            }
        }
    ...
    private function rememberSourceHash(array &$hashes, string $path, string $sourceHash): void
    {
        if (isset($hashes[$path]) && !hash_equals($hashes[$path], $sourceHash)) {
            throw new RuntimeException('Method removal plan contains contradictory source hashes for ' . $path . '.');
        }
    ...
    private function sourcePath(string $root, string $relative): string
    {
        ...
        if (!is_string($realRoot) || !is_string($path) || !$this->insideRoot($realRoot, $path)) {
            throw new RuntimeException('Method removal source path escapes the map root: ' . $relative);
        }
    ...
    private function relativePath(string $relative): string
    {
        if ($relative === '' || str_contains($relative, "\0")) {
            throw new RuntimeException('Method removal plan contains an invalid source path.');
        }
        $relative = str_replace('\\', '/', $relative);
        if (str_starts_with($relative, '/') || preg_match('~^[A-Za-z]:/~', $relative) === 1) {
            throw new RuntimeException('Method removal plan path must be relative to the map root: ' . $relative);
        }
        foreach (explode('/', $relative) as $segment) {
            if ($segment === '' || $segment === '.' || $segment === '..') {
                throw new RuntimeException('Method removal plan contains a non-canonical relative path: ' . $relative);
            }
        }
    ...
    {
        $temporary = $path . '.agent-loop-method-removal-' . $purpose . '-' . bin2hex(random_bytes(8));
        if (file_exists($temporary) || is_link($temporary)) {
            throw new RuntimeException('Method-removal temporary path already exists: ' . $temporary);
        }
    ...
    private function lint(string $path): array
    {
        ...
        if (!is_resource($process)) {
            throw new RuntimeException('Unable to start PHP syntax validation for staged method-removal source.');
        }
    ...
    private function read(string $path): string
    {
        ...
        if (!is_string($content)) {
            throw new RuntimeException('Unable to read method-removal source: ' . $path);
        }
