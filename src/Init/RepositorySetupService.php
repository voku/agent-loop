...
final readonly class RepositorySetupService
{
    ...
    public function uninstall(
        ...
        if ($plan->agent === 'claude'
            && $plan->withHooks
            && $this->containsClaudeHookReceiptRemoval($plan->operations)
        ) {
            try {
        ...
        if ($plan->agent === 'claude' && $plan->withHooks && $this->containsClaudeHookReceiptRemoval($assetOperations)) {
            try {
                ...
                $reason = 'Claude hook registrations changed after planning; hook assets were left untouched: '
                    . $exception->getMessage();
                $retained = [];
                foreach ($assetOperations as $operation) {
                    ...
                    $registrationBlocked[] = new ManagedAssetOperation(
        ...
        $runtimeBlocked = [
            ...$instructionOutcome['blocked'],
            ...$registrationBlocked,
            ...$assetOutcome['blocked'],
        ];
    ...
    private function runtimeBoundary(string $host): string
    {
        return match ($host) {
            ...
            'cursor' => 'agent-loop projects AGENTS.md, native Cursor skills and agents, read-only role metadata, and a fail-closed beforeShellExecution shell authority guard in .cursor/hooks.json. Cursor CLI policy and MCP authority coverage are separate and unclaimed. Runtime auto-detection and live hook/read-only enforcement remain unverified; select Cursor explicitly until Slice 3 supplies runtime evidence.',
