...
final readonly class ClaudeHookRegistrationProjector
{
    ...
    public function removeOwnedRegistrations(bool $dryRun = false): bool
    {
        ...
        if (!$dryRun && $changed) {
            $receiptContent = $this->store->readReceiptContent();
            if ($receiptContent === null) {
                throw new InvalidArgumentException('Claude hook registration receipt disappeared during deregistration.');
            }
            $this->store->commit($this->settingsContent($settings, $hooks), $receiptContent);
        }
