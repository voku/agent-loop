    ...
    private function checkRecallOutcomeGate(TaskContract $contract, string $learningRoot): array
    {
        ...
        foreach ($selections as $selection) {
            if (
                ...
                && $selection->selected
                && $selection->outcomeWithheldReason !== null
                && trim($selection->outcomeWithheldReason) !== ''
            ) {
                $withheld[$selection->guidanceId] = true;
            }
        }
        $withheld = array_intersect_key($withheld, $selectedSet);
