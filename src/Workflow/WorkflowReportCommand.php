    ...
    private function loggedOutcomeCount(string $taskId, ?string $learningRoot): int
    {
        ...
        try {
            return count(array_filter(
                (new OutcomeRepository())->loadAll($learningRoot),
                static fn (array $outcome): bool => ($outcome['task_id'] ?? null) === $taskId,
            ));
        } catch (RuntimeException) {
            // The Learning section separately reports invalid owner state. Keep
            // this Recall summary available without inventing a second parser.
            return 0;
        }
    }
