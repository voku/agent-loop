...
final readonly class RunManifestProjector
{
    ...
    private function recallReference(
        ...
        if ($output === null) {
            return [
                ...
                'state' => 'missing',
                'observation_mode' => 'checked',
                'path' => RecallOutputRoot::relativeTo($this->rootPath, $identityPath),
            ];
        }
    ...
    private function recallBindingFailure(
        ...
        if (!$output->hasBundle()) {
            return $output->snapshotSha256() !== null
                ? 'Recall output is incomplete for the current governed Run: compiled bundle is missing.'
                : null;
        }
        if (!$output->isBundleReadable()) {
            return 'Recall bundle is unreadable for the current governed Run.';
        }
        if (!$output->bindsTo($run->taskId, $run->contractRevision) || !$output->bindsTo($contract->taskId, $contract->revision)) {
            return sprintf(
