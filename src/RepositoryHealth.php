...
/** Pure interpretation of repository-maintenance evidence collected elsewhere. */
final class RepositoryHealth
{
    ...
    public function classify(array $evidence): array
    {
        ...
        $expected = $composer && ($active || $config !== null || ($comparable >= 3 && $peerActive * 4 >= $comparable * 3));
        ...
        if (!$active && $config === null) {
            ...
            !$active
            && ($evidence['fork'] ?? null) === true
            && ($config !== 'renovate.json' || ($renovate['fork_processing'] ?? null) !== 'enabled')
        ) {
